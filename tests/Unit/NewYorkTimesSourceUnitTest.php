...
class NewYorkTimesSourceUnitTest extends TestCase
{
        use RefreshDatabase;

    public function test_newyorktimes_news_can_fetch_data(): void
    {
        $this->assertContains(PossibleNewsSource::NEWYORKTIMES->value, config('innoscripta.supported_news_sources'));

        $source = (new NewsSourceFactory())->make(PossibleNewsSource::NEWYORKTIMES->value);
    ...
    public function test_newyorktimes_news_data_can_adapt_to_its_features(): void
    {
        $this->assertContains(PossibleNewsSource::NEWYORKTIMES->value, config('innoscripta.supported_news_sources'));
    ...
    public function test_newyorktimes_news_data_can_orchestrate_news_fetch(): void
    {
        $this->assertContains(PossibleNewsSource::NEWYORKTIMES->value, config('innoscripta.supported_news_sources'));
